...
const PostCreatePage = ({ type }) => {
	...
	const onSubmit = async (payload) => {
		...
		const { failed, err, pathname } = await createPost(cookies.token, payload);
		...
		router.push({
			...
			query: {
				toast: "success",
				msg: encodeURIComponent(`Successfully Created New ${type} Post`),
			},
		});
	};
	...
	return (
		<MainLayout>
			<Container>
				<EditProvider>
					<EditContainer onSubmit={onSubmit} value={{ type }} error={err} />
				</EditProvider>
			</Container>
		</MainLayout>
	);
